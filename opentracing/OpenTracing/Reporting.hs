...
consumer opt@BatchOptions{..} q = async . handleAsync drain . forever $
  ...
  where
    go onEmpty = do
        ...
        case batch of
            [] -> onEmpty
            xs -> uninterruptibleMask_ $ do
                      to <- timeout timeoutMicros
                          . handleAny (logErr opt . ErrReporterException)
                          $ _boptReporter xs
                      case to of
                          Nothing -> logErr opt ErrReporterTimeout
                          Just _  -> return ()

    drain ThreadKilled = logErr opt ErrReporterCancelled *> go (return ())
...
logErr BatchOptions{_boptErrorLog=errlog} e = errlog $ msg e <> nl
  where
    ...
    msg = \case
        ...
        ErrReporterCancelled         -> sbs "Batch reporter cancelled, commencing queue drain"
